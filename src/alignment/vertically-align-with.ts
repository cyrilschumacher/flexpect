  ...
  switch (alignment) {
    ...
    default: {
      ...
      const containerCenter =
        containerBoundingBox.y + containerBoundingBox.height / 2;
      const elementCenter =
        receivedBoundingBox.y + receivedBoundingBox.height / 2;
...
export async function toBeVerticallyAlignedWith(
  ...
  const delta = computeVerticalDelta(
    alignment,
    receivedBoundingBox,
  ...
  return {
    pass,
    message: () =>
      formatVerticalAlignmentMessage(
        pass,
        alignment,
        tolerancePercent,
        delta,
